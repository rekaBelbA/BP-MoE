    ...
    def prep_input_mails(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                b.srcdata['mem'] = self.node_memory[b.srcdata['ID'].long()].cuda() #[Len(b.srcdata['ID']) * 100]
                #a=b.srcdata['mem']#
                b.srcdata['mem_ts'] = self.node_memory_ts[b.srcdata['ID'].long()].cuda() #[Len(b.srcdata['ID'])] 
                #bs=b.srcdata['mem_ts'] #
                b.srcdata['mem_input'] = self.mailbox[b.srcdata['ID'].long()].cuda().reshape(b.srcdata['ID'].shape[0], -1) #[Len(b.srcdata['ID']),372] 
                #ma=b.srcdata['mem_input']#
                b.srcdata['mail_ts'] = self.mailbox_ts[b.srcdata['ID'].long()].cuda() #[Len(b.srcdata['ID']),1]
                #ssa=b.srcdata['mail_ts']#
    ...
    def update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, block, neg_samples=1):
        with torch.no_grad():
            ...
            if self.memory_param['deliver_to'] == 'self':
                ...
                # elif self.memory_param['mail_combine'] == 'mean':
