...
for idx, row in tqdm(df.iterrows(), total=len(df)):
    ...
    ext_full_indices[src].append(dst) # node vi:[u1,u2,...]
    ext_full_ts[src].append(row['time'])
    ext_full_eid[src].append(idx) # edge number
    if args.add_reverse:
